-- Fix the get_decrypted_calendar_tokens function to properly cast json to jsonb
-- This fixes the type mismatch error when calling create_audit_log
...
BEGIN
  ...
  -- For now, return tokens as-is (assuming they're already in usable format)
  -- TODO: Implement proper decryption when encryption functions are available
  decrypted_access := integration_record.access_token;
  
  IF integration_record.refresh_token IS NOT NULL THEN
    decrypted_refresh := integration_record.refresh_token;
  END IF;
