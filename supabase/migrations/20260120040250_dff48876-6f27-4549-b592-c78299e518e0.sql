-- Create batch goal progress calculation function
-- This eliminates N+1 query pattern by calculating progress for multiple goals at once

CREATE OR REPLACE FUNCTION calculate_goals_progress_batch(p_goal_ids uuid[])
RETURNS TABLE (
  goal_id uuid,
...
BEGIN
  ...
  -- Count completed linked tasks per goal
  completed_counts AS (
    SELECT 
      glt.goal_id,
      COUNT(DISTINCT glt.task_id) as completed_count
    FROM goal_linked_tasks glt
  ...
  SELECT 
    gd.id as goal_id,
    CASE 
      WHEN gd.goal_type = 'target' THEN COALESCE((gd.success_criteria->>'current_value')::numeric, 0)
    ...
    CASE 
      WHEN gd.goal_type = 'target' THEN COALESCE((gd.success_criteria->>'target_value')::numeric, 1)
    ...
    END as target_value,
    CASE 
      WHEN gd.goal_type = 'target' THEN 
        LEAST(100, ROUND(COALESCE((gd.success_criteria->>'current_value')::numeric, 0) / 
          NULLIF(COALESCE((gd.success_criteria->>'target_value')::numeric, 1), 0) * 100, 0))
