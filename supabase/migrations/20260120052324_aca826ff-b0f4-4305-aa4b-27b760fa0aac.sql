-- Fix: Update calculate_goals_progress_batch to handle 'target_count' goal_type
...
BEGIN
  ...
  SELECT 
    gd.id as goal_id,
    CASE 
      WHEN gd.goal_type IN ('target', 'target_count') THEN COALESCE((gd.success_criteria->>'current_value')::numeric, 0)
    ...
    CASE 
      WHEN gd.goal_type IN ('target', 'target_count') THEN COALESCE((gd.success_criteria->>'target_value')::numeric, 1)
    ...
    END as target_value,
    CASE 
      WHEN gd.goal_type IN ('target', 'target_count') THEN 
