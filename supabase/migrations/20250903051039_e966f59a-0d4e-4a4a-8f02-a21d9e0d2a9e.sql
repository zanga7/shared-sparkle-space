-- Fix the set_child_pin function to use correct audit log parameters
DROP FUNCTION IF EXISTS public.set_child_pin(uuid, text, text);

-- Create function to set child PIN with proper hashing and correct audit log call
...
BEGIN
  ...
  -- Create audit log with correct parameter types
  PERFORM public.create_audit_log(
    ...
    profile_id_param,
    NULL,
    json_build_object('pin_type', pin_type_param, 'set_at', NOW())::jsonb
  );
