-- First, remove duplicate contributions keeping only the first one for each profile/reward pair
WITH duplicate_contributions AS (
  SELECT id, 
         ROW_NUMBER() OVER (PARTITION BY profile_id, reward_id ORDER BY contributed_at) as rn
  FROM public.group_contributions
)
DELETE FROM public.group_contributions 
WHERE id IN (
  SELECT id FROM duplicate_contributions WHERE rn > 1
);
