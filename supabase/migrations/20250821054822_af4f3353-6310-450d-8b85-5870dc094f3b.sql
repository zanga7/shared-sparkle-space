...
ALTER TABLE public.rewards ALTER COLUMN reward_type DROP DEFAULT;

-- Update rewards table to use the new enum, handle null values
UPDATE public.rewards SET reward_type = 'always_available' WHERE reward_type IS NULL;
ALTER TABLE public.rewards ALTER COLUMN reward_type TYPE public.reward_type USING 
  CASE 
    WHEN reward_type = 'once_off' THEN 'once_off'::public.reward_type
    WHEN reward_type = 'always_available' THEN 'always_available'::public.reward_type
    ELSE 'always_available'::public.reward_type
  END;

-- Set the new default value
ALTER TABLE public.rewards ALTER COLUMN reward_type SET DEFAULT 'always_available'::public.reward_type;
