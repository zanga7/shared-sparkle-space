-- Drop functions with parameter mismatches and recreate them properly
DROP FUNCTION IF EXISTS public.create_missing_profile_for_user(uuid);
DROP FUNCTION IF EXISTS public.authenticate_child_pin(uuid, text);
DROP FUNCTION IF EXISTS public.set_child_pin(uuid, text);

-- Recreate functions with correct parameter names and search_path
