-- Phase 1: Emergency Data Cleanup & Safety

-- Add unique constraints to prevent duplicate tasks
ALTER TABLE tasks ADD CONSTRAINT unique_series_due_date 
  UNIQUE (series_id, due_date) 
  WHERE series_id IS NOT NULL;

-- Add unique constraint for rotating tasks (using title as identifier for now)
ALTER TABLE tasks ADD CONSTRAINT unique_rotating_task_date 
  UNIQUE (title, assigned_to, due_date) 
  WHERE title LIKE '%rotating%' OR task_group = 'rotating';
...
-- Add check constraint for valid task_group values
