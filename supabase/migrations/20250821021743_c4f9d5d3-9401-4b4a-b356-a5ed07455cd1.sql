-- Drop the view and recreate it as a proper view with better security
DROP VIEW IF EXISTS public.calendar_security_summary;

-- Create a secure function to get calendar security summary
CREATE OR REPLACE FUNCTION public.get_calendar_security_summary()
RETURNS TABLE(
  id uuid,
  ...
  owner_name text,
  access_count_7_days bigint,
  failed_access_count_7_days bigint
) 
LANGUAGE plpgsql
SECURITY DEFINER
...
BEGIN
  -- Get current user's family ID and verify they are a parent
  SELECT p.family_id INTO current_user_family_id
  FROM public.profiles p
  WHERE p.user_id = auth.uid() AND p.role = 'parent';

  IF current_user_family_id IS NULL THEN
    RAISE EXCEPTION 'Access denied: Only parents can view security summary';
  END IF;
