...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (req.method === 'POST') {
      ...
      const { data: googleClientId, error: clientIdError } = await supabaseClient
        .rpc('get_oauth_credential', { credential_key: 'google_client_id' });
      
      const { data: googleClientSecret, error: clientSecretError } = await supabaseClient
      ...
      if (clientIdError || clientSecretError || !googleClientId || !googleClientSecret) {
        throw new Error('Google OAuth credentials not configured in super admin');
      }
