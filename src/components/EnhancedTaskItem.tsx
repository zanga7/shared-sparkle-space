...
export const EnhancedTaskItem = ({
  ...
  const {
    data: goalConnections
  } = useTaskGoalConnections(task.id, (task as any).series_id, task.rotating_task_id);
  ...
  const firstCompleterProfile = firstCompletion ? familyMembers.find(m => m.id === firstCompletion.completed_by) : null;
  ...
  const isCompletedByMe = task.completion_rule === 'any_one' ? (task.task_completions || []).some(c => c.completed_by === checkMemberId) // Did I complete it?
  ...
  const isCompleted = task.completion_rule === 'any_one' ? hasAnyCompletion : isCompletedByMe; // Each person sees their own completion state
  ...
  const {
    styles: colorStyles
  } = useMemberColor(memberColor);
  return <div className={cn("group/task relative rounded-lg p-3 transition-all hover:shadow-md", !isCompleted && !isCompletedBySomeoneElse && !(task as any).isVirtual && "cursor-grab active:cursor-grabbing", isUnassigned && "bg-muted/30", (isCompleted || isCompletedBySomeoneElse) && "opacity-60" // Grey out all completed tasks consistently
  )} style={!isUnassigned ? isCompleted || isCompletedBySomeoneElse ? colorStyles.bg20 : colorStyles.bg50 : undefined}>
      <div className="flex items-start gap-2">
        {/* Complete/Uncomplete Button */}
        <Button size="sm" variant={isCompletedByMe ? "default" : "outline"} onClick={e => {
        e.stopPropagation();
        ...
        <div className="flex-1 min-w-0 space-y-1">
          ...
          <div className="flex items-start justify-between gap-2">
            ...
            {showActions && !isCompleted && <div className="invisible opacity-0 group-hover/task:visible group-hover/task:opacity-100 transition-all duration-200 flex items-center gap-1 shrink-0">
                {onEdit && <Button size="sm" variant="ghost" className="h-6 w-6 p-0 hover:bg-accent" onClick={e => {
              e.stopPropagation();
              onEdit(task);
            }} onMouseDown={e => e.stopPropagation()} onTouchStart={e => e.stopPropagation()}>
                    <Edit className="h-3 w-3" />
                  </Button>}
                {onDelete && <Button size="sm" variant="ghost" className="h-6 w-6 p-0 hover:bg-destructive/10 hover:text-destructive" onClick={e => {
              e.stopPropagation();
              onDelete(task);
            }} onMouseDown={e => e.stopPropagation()} onTouchStart={e => e.stopPropagation()}>
          ...
          <div className="flex items-center gap-1 flex-wrap">
            ...
            <span className="text-[0.72rem] h-5 flex items-center gap-1 text-primary">
              <Target className="h-2.5 w-2.5" />
              {task.points} pts
              {(() => {
              // Handle both regular tasks and virtual tasks
              const virtualTask = task as any;
              const assigneeCount = task.assignees?.length || virtualTask.assigned_profiles?.length || (task.assigned_profile ? 1 : 0);
              if (assigneeCount > 1) {
            ...
            {isCompletedBySomeoneElse && firstCompleterProfile && <Badge variant="secondary" className="text-[0.625rem] py-0 h-4 flex items-center gap-0.5 bg-foreground/[0.08] border-0">
                <UserCheck className="h-2 w-2" />
                Completed by {firstCompleterProfile.display_name}
              </Badge>}

            {/* Assignees Display - Hidden when completed */}
            {!isCompleted && !isCompletedBySomeoneElse && <TaskAssigneesDisplay task={task} showNames={false} onClick={onEdit ? () => onEdit(task) : undefined} />}

            {/* Due Date - Hidden when completed */}
            {!isCompleted && task.due_date && <Badge variant={isOverdue ? "destructive" : daysUntilDue === 0 ? "default" : "outline"} className={cn("text-[0.625rem] py-0 h-4 flex items-center gap-0.5", !isOverdue && daysUntilDue !== 0 && "bg-foreground/[0.08] border-0")}>
                {isOverdue ? <AlertTriangle className="h-2 w-2" /> : <Calendar className="h-2 w-2" />}
                {isOverdue ? 'Overdue' : daysUntilDue === 0 ? 'Today' : daysUntilDue === 1 ? 'Tomorrow' : format(new Date(task.due_date), "MMM d")}
              </Badge>}
            ...
            {!task.rotating_task_id && (task.completion_rule === 'everyone' || task.completion_rule === 'any_one') && Math.max(task.assignees?.length ?? 0, typeof (task as any).series_assignee_count === 'number' ? (task as any).series_assignee_count : 0, Array.isArray((task as any).assigned_profiles) ? (task as any).assigned_profiles.length : 0, task.assigned_to ? 1 : 0) > 1 && <Badge variant="secondary" className="text-[0.625rem] py-0 h-4 flex items-center gap-0.5 border-0 bg-slate-50/25">
                ...
                <Tooltip>
                  ...
                  <TooltipContent side="top" className="max-w-xs">
                    <div className="space-y-1">
                      {goalConnections.map((gc, idx) => <div key={idx} className="text-xs">
                          <span className="font-medium">{gc.goalTitle}</span>
                          {gc.milestoneTitle && <span className="text-muted-foreground"> → {gc.milestoneTitle}</span>}
                        </div>)}
                    </div>
                  </TooltipContent>
                </Tooltip>
