...
const MemberStatCard = memo(({
  ...
  return <motion.div initial={{
    ...
    duration: 0.2
      ...
      <Card className="relative overflow-hidden cursor-pointer group transition-all duration-300 hover:shadow-xl border-0" onClick={onViewDashboard}>
        <div className="flex">
          ...
          <div className="w-1/4 flex flex-col items-center justify-center p-3 bg-muted/50 rounded-l-lg">
            <UserAvatar name={member.display_name} color={member.color} avatarIcon={member.avatar_url || undefined} size="lg" className="w-14 h-14" />
          ...
          <div className="w-3/4 relative" style={{
          backgroundColor: hex
        }}>
              ...
              <div className="space-y-1.5 mb-2">
                <div className="flex items-center justify-between text-sm">
                  <div className="flex items-center gap-1.5 text-white/80">
                    ...
                    <span>Tasks</span>
                    {progressPercent === 100 && todayTaskCount > 0 && <span className="ml-1">🎉</span>}
                  </div>
  ...
  return <div className="h-full flex flex-col">
      <div className="pb-4 pt-4 px-4">
        <div className="flex items-center justify-between">
          ...
          {onViewCalendar && <Button variant="outline" size="sm" onClick={onViewCalendar} className="gap-2">
      ...
      <div className="flex-1 px-4 pb-4 overflow-y-auto">
        {todayEvents.length === 0 ? <div className="text-center py-6 text-muted-foreground">
            <Calendar className="h-10 w-10 mx-auto mb-2 opacity-30" />
            <p className="text-sm">No events scheduled for today</p>
          </div> : <div className="space-y-3 max-h-[400px] overflow-y-auto">
            {todayEvents.map((event, index) => <motion.div key={event.id} initial={{
          opacity: 0,
          x: -10
        }} animate={{
          opacity: 1,
          ...
          delay: index * 0.1
                ...
                {event.attendees && event.attendees.length > 0 && <div className="flex gap-1">
                    {event.attendees.slice(0, 3).map((attendee: any) => <UserAvatar key={attendee.profile_id} name={attendee.profile?.display_name || 'Unknown'} color={attendee.profile?.color} avatarIcon={attendee.profile?.avatar_url || undefined} size="sm" />)}
                    {event.attendees.length > 3 && <div className="w-6 h-6 rounded-full bg-muted flex items-center justify-center text-xs">
...
const RewardsWidget = memo(({
  ...
  return <div>
      ...
      <div className="pt-0 px-4 pb-4">
        {activeRewards.length === 0 ? <div className="text-center py-4 text-muted-foreground">
            <Gift className="h-8 w-8 mx-auto mb-2 opacity-30" />
            <p className="text-sm">No rewards available</p>
          </div> : <div className="space-y-2 max-h-[200px] overflow-y-auto">
            {activeRewards.slice(0, 4).map((reward, index) => <motion.div key={reward.id} initial={{
          opacity: 0,
          x: 10
        }} animate={{
          opacity: 1,
          ...
          delay: index * 0.1
        }} className="flex items-center gap-2 p-2 rounded-lg bg-muted/50 hover:bg-muted transition-colors px-[10px] py-[10px]">
                <div className="flex-shrink-0">
                  <div className="w-8 h-8 rounded-full bg-amber-500/10 flex items-center justify-center">
                    {reward.image_url ? <img src={reward.image_url} alt={reward.title} className="w-6 h-6 rounded-full object-cover" /> : <Gift className="h-4 w-4 text-amber-500" />}
                  </div>
                </div>
                <div className="flex-1 min-w-0">
            ...
            {activeRewards.length > 4 && <p className="text-xs text-center text-muted-foreground pt-1">
...
const CompactConsistencyGrid = memo(({
  goal
}: {
  goal: any;
}) => {
  ...
  return <div className="flex gap-0.5">
      {boxes.map((isCompleted, i) => <div key={i} className={`w-3 h-3 rounded-sm ${isCompleted ? 'bg-green-500' : i === boxes.length - 1 ? 'bg-muted ring-1 ring-primary/50' : 'bg-muted/50'}`} />)}
    </div>;
});
CompactConsistencyGrid.displayName = 'CompactConsistencyGrid';

// Goals widget
const GoalsWidget = memo(({
  goals,
  onNavigateToGoals
}: {
  goals: any[];
  ...
  return <div className="h-full flex flex-col">
      <div className="pb-4 pt-4 px-4">
        <div className="flex items-center justify-between">
          ...
          {onNavigateToGoals && <Button variant="outline" size="sm" onClick={onNavigateToGoals} className="gap-2">
  ...
  const memberStats = useMemo(() => {
    ...
    return familyMembers.map(member => {
      const memberTasks = tasks.filter(task => {
        ...
        const isAssigned = task.assigned_to === member.id || task.assignees?.some(a => a.profile_id === member.id);
      ...
      const completedTasks = memberTasks.filter(task => task.task_completions && task.task_completions.length > 0);
      return {
        member,
          ...
          <div>
            <div className="pb-4 pt-4 px-4">
              <div className="flex items-center justify-between">
                ...
                <Button variant="outline" size="sm" onClick={onNavigateToTasks} className="gap-2">
            ...
            <div className="px-4 space-y-3">
              {sortedMemberStats.map(stat => <MemberStatCard key={stat.member.id} member={stat.member} todayTaskCount={stat.todayTaskCount} completedCount={stat.completedCount} todayEventCount={stat.todayEventCount} onViewDashboard={() => onMemberSelect(stat.member.id)} />)}
            </div>
          </div>
